...
export default function CourseDetailPage() {
  ...
  const description = `Study ${course.code} ${course.title} with Nexus Education Private School. Review its Grade ${course.grade} ${course.type} description, prerequisite, credit details, outline and registration options.`;
  ...
  return (
    <>
      ...
      <section className="course-detail-hero">
        <div className="container">
          ...
          <div className="course-hero-grid">
            <div className="course-hero-copy"><span className="eyebrow">ONTARIO COURSE · GRADE {course.grade}</span><span className="course-code course-code-large">{course.code}</span><h1>{course.title} <span>| {course.code}</span></h1><p>{course.description}</p><div className="course-hero-badges"><span>Grade {course.grade}</span><span>{course.type}</span><span>{course.credit} credit</span></div><div className="button-row"><AddToCartButton course={course} /><button className="btn btn-secondary" type="button" onClick={registerCourse}><GraduationCap size={17} /> Register Now</button><Link className="text-link" to={`/courses/${course.slug}/outline`}><ClipboardList size={17} /> Course Outline</Link></div></div>
            <div className="course-hero-side"><div className="course-visual-card"><img src={visual} alt={`${course.department} visual for ${course.title}`} fetchPriority="high" /><div><span>{course.department}</span><strong>{course.code}</strong></div></div><div className="course-snapshot"><span className="mini-label">COURSE INFORMATION</span><dl><div><dt>Course code</dt><dd>{course.code}</dd></div><div><dt>Grade</dt><dd>{course.grade}</dd></div><div><dt>Course type</dt><dd>{course.type}</dd></div><div><dt>Credit value</dt><dd>{course.credit}</dd></div><div><dt>Department</dt><dd>{course.department}</dd></div></dl></div></div>
          </div>
        </div>
      </section>
      ...
      <Reveal as="section" className="section container course-detail-body course-detail-expanded">
        <article>
          <SectionHeading eyebrow="COURSE OVERVIEW" title="Understand the Course Before You Register." />
          <p className="large-copy">{course.description}</p>
          <h2>Prerequisite</h2><div className="notice-card"><BookOpenCheck size={22} /><div><strong>{course.prerequisite}</strong><p>Confirm that the listed prerequisite and current eligibility rules apply to the student's record before registration.</p></div></div>
          ...
          <h2>Course outline</h2><p>The separate outline page mirrors the current Nexus LMS section sequence and adds a responsive unit table. It does not replace registered course instructions.</p><Link className="outline-link-card" to={`/courses/${course.slug}/outline`}><span className="icon-bubble"><ClipboardList size={22} /></span><span><strong>Open the {course.code} course outline</strong><small>{course.outline.length} LMS sections listed</small></span><ArrowRight size={18} /></Link>
        ...
        <aside><div className="planning-card course-registration-panel"><ShoppingCart size={30} /><span className="mini-label">COURSE REGISTRATION</span><h2>{course.code}</h2><dl><div><dt>Credit</dt><dd>{course.credit}</dd></div><div><dt>Prerequisite</dt><dd>{course.prerequisite}</dd></div></dl><div className="availability-note"><CheckCircle2 size={18} /><span><strong>{course.offeringStatus}</strong>The catalogue record does not guarantee current availability, delivery or individual eligibility.</span></div><AddToCartButton course={course} /><button className="btn btn-secondary" type="button" onClick={registerCourse}><GraduationCap size={17} /> Register Now</button><Link className="text-link" to={`/inquiry?course=${course.code}`}>Ask About This Course</Link><a className="text-link" href={brand.lms} target="_blank" rel="noreferrer">Registered student LMS <ExternalLink size={15} /></a><div className="course-plan-points"><span><Route size={17} />Check the prerequisite chain</span><span><GraduationCap size={17} />Confirm the intended pathway</span></div></div></aside>
      </Reveal>
