...
export default function CoursesPage({ gradeOverride = "" }) {
  ...
  return (
    <>
      <PageHero eyebrow="ACADEMICS" title={routeGrade ? `Grade ${routeGrade} Courses` : "Find the Course That Moves You Forward."} text="Search the Nexus catalogue by exact course code, title, grade, course type or department. Exact code matches rank first." metaTitle={routeGrade ? `Grade ${routeGrade} Ontario Courses | Nexus Education` : undefined} metaDescription={routeGrade ? `Explore ${filtered.length} Grade ${routeGrade} Ontario secondary course records in the Nexus Education catalogue.` : undefined}><Link className="btn btn-primary" to="/register">Register for Courses <ArrowRight size={17} /></Link><Link className="btn btn-secondary" to="/academic-planning">Plan Your Path</Link></PageHero>
