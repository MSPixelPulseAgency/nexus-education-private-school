...
export default function CoursesPage({ gradeOverride = "" }) {
  ...
  return (
    <>
      <PageHero eyebrow="ACADEMICS" title={routeGrade ? `Grade ${routeGrade} Courses` : "Find the Course That Moves You Forward."} text="Search the Nexus catalogue by exact course code, title, grade, course type or department. Exact code matches rank first." metaTitle={routeGrade ? `Grade ${routeGrade} Courses | Nexus Education` : undefined} metaDescription={routeGrade ? `Explore ${filtered.length} Grade ${routeGrade} course records in the Nexus Education Private School catalogue.` : undefined}>
        <Link className="btn btn-primary" to="/inquiry">Ask About a Course <ArrowRight size={17} /></Link>
        <Link className="btn btn-secondary" to="/academic-planning">Plan Your Path</Link>
      </PageHero>
      <Reveal as="section" className="section container course-marketplace">
        <h2 className="sr-only">Course catalogue results</h2>
        <div className="catalog-toolbar">
          <div className="catalog-search"><Search size={20} /><label className="sr-only" htmlFor="catalog-search">Search courses</label><input id="catalog-search" value={query} onChange={(event) => update("q", event.target.value)} placeholder="Search CGC1W, Grade 12, Technology…" /></div>
          <div className="catalog-count"><strong>{filtered.length}</strong><span>course{filtered.length === 1 ? "" : "s"} found</span></div>
        </div>
        <div className="catalog-layout">
          <aside className="catalog-filters" aria-label="Course filters">
            ...
            <div className="grade-filter-links"><span>Browse by grade</span>{[9,10,11,12].map((item) => <Link className={String(item) === routeGrade ? "active" : ""} key={item} to={`/courses/grade-${item}`}>Grade {item}<ArrowRight size={14} /></Link>)}</div>
          </aside>
          <div className="catalog-results">
            <div className="active-filters" aria-label="Active filters">{query && <button onClick={() => update("q", "")}>Search: {query} <X size={14} /></button>}{!routeGrade && grade && <button onClick={() => update("grade", "")}>Grade {grade} <X size={14} /></button>}{type && <button onClick={() => update("type", "")}>{type} <X size={14} /></button>}{department && <button onClick={() => update("department", "")}>{department} <X size={14} /></button>}{!hasFilters && <span><Filter size={15} /> Showing the full catalogue</span>}</div>
            {filtered.length ? <><div className="card-grid two catalog-grid">{filtered.slice(0, visible).map((course) => <CourseCard course={course} key={course.code} />)}</div>{visible < filtered.length && <div className="section-action"><button className="btn btn-secondary" type="button" onClick={() => setVisible((count) => count + PAGE_SIZE)}>Load More Courses <ArrowRight size={16} /></button><span>{Math.min(visible, filtered.length)} of {filtered.length}</span></div>}</> : <div className="empty-state"><Search size={34} /><h2>No courses match those filters.</h2><p>Try another code, title, grade or department—or clear the active filters.</p><button className="btn btn-primary" type="button" onClick={reset}>Clear Filters</button></div>}
          </div>
        </div>
      </Reveal>
      <CTASection />
